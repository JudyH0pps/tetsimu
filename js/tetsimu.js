...
(function() {
  ...
  var ControllerStore = $.extend({
    ...
    _updateSpecialKeys: function(action, isDown) {
      // 複数ある修飾キーを同時押下した場合、一つのkeyUpで押下状態が클리어される
...
(function() {
  ...
  var ReplayStore = $.extend({
    ...
    changeModeToSimu: function(action) {
      ...
      // リプレイモード→プレイング모드 변경時は、前回切替前の続きから再開
...
var ConfigPanel = React.createClass({displayName: "ConfigPanel",
  ...
  render: function() {
    var simuKeyConfigItems = this._makeKeyConfigItems([
      ...
      { actionName: '한 수 앞으로', action: 'forward'},
      { actionName: '한 수 뒤로', action: 'back'},
      { actionName: '리트라이', action: 'retry'},
      { actionName: '슈퍼 리트라이', action: 'superRetry'},
    ...
    var replayKeyConfigItems = this._makeKeyConfigItems([
      ...
      { actionName: '１手되돌리기', action: 'back'},
      { actionName: '最初に되돌리기', action: 'backToHead'},
      { actionName: '모드 변경(Simu)', action: 'changeModeToSimu'},
      { actionName: '되돌리기', action: 'cancel'},
      { actionName: 'URL출력', action: 'createUrlParameters'},
      { actionName: '설정', action: 'configure'}
    ], this.state.config.key.replay);

    var editKeyConfigItems = this._makeKeyConfigItems([
      { actionName: 'I 선택', action: 'selectTypeI'},
      { actionName: 'J 선택', action: 'selectTypeJ'},
      { actionName: 'L 선택', action: 'selectTypeL'},
      { actionName: 'O 선택', action: 'selectTypeO'},
      { actionName: 'S 선택', action: 'selectTypeS'},
      { actionName: 'T 선택', action: 'selectTypeT'},
      { actionName: 'Z 선택', action: 'selectTypeZ'},
      { actionName: '흰색 선택', action: 'selectTypeOjama'},
      { actionName: '검은색 선택', action: 'selectTypeNone'},
    ...
    return React.createElement("div", {className: "config-panel"}, 
        React.createElement("div", {className: "inner-config-panel"}, 
          ...
          React.createElement("div", {className: "config-button-area"}, 
            React.createElement("button", {onClick: this.onSave}, "저장"), 
            React.createElement("button", {onClick: this.onCancel}, "취소"), 
            React.createElement("button", {onClick: this.onInitialize}, "초기설정으로 되돌리기")
          )
        ), 
  ...
  _validateConfig: function(name, config, validationState) {
    ...
    for (p in config) {
      ...
      if ((key !== '') && (key in appearedKey) && !(key in duplicatekeys)) {
        ...
        errorMessages.push(key + '가 중복되어 있습니다');
      }
...
var MainPanel = React.createClass({displayName: "MainPanel",
  ...
  _restoreConfig: function() {
    ...
    // 설정情報をローカルストレージより取得（なければデフォルトを使用）
    try {
    ...
    return React.createElement("div", {className: "operation-panel"}, 
        ...
        React.createElement("div", {className: "operation-mode-panel"}, 
          React.createElement("div", {className: "operation-simu-panel"}, 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onClear}, "클리어"), 
            ...
            (!!this.state.before) ?
              React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onCancel}, "되돌리기")
              : "", 
            ...
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onCreateUrlParameters}, "URL출력"), 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onConfigure}, "설정"), 
    ...
    return React.createElement("div", {className: "operation-panel"}, 
        ...
        React.createElement("div", {className: "operation-mode-panel"}, 
          React.createElement("div", {className: "operation-simu-panel"}, 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onBack}, "<"), 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onForward}, ">"), 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onRetry}, "리트라이"), 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onSuperRetry}, "슈퍼리트라이"), 
            React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onClear}, "클리어"), 
            ...
            (!!this.state.before) ?
              React.createElement("a", {className: "operation-btn", href: "javascript:void(0)", onClick: this.onBackToEditMode}, "되돌리기")
              : "", 
